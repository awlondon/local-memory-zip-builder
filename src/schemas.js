...
export function buildInstructionsFile() {
  return [
    ...
    "3) If you need to regenerate or refine that graph with an LLM agent, open core-obsessions-agent-README.md.",
    "4) Keep the local_memory/ directory intact (do not rename internal folders).",
    "5) Point your local coding or LLM agent at local_memory/.",
    "6) Ask the agent to read manifest/corpus.json and index/*.json before opening large payload shards.",
    "7) Load index/query_protocol.json when you need concept adjacency, symbol mappings, or chunk lookup tables.",
    "8) Prefer speaker_role, turn_role, speaker_inference_source, and speaker_confidence before re-inferring identity from raw text.",
    "9) Use symbolic/*.stream.jsonl for fast retrieval cues and textpack/ for exact chunk reconstruction, including speaker metadata.",
    "10) Open chunks/chunk_text_part_*.jsonl only as a compatibility fallback or when speaker_role is unknown or mixed.",
    "11) Open raw session shards only for grounded span-level inspection when they are present.",
    "",
    "Notes:",
